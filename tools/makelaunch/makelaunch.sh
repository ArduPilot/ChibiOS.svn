...
#  ELFRELPATH=$(realpath --relative-to=$P1 $P2)
  ...
  mkdir -p $1/$DEBUGDIR
  cat ./arm_flash-and-run.launch.template | sed -r "s|EEEEEEEE|$ELFRELPATH|g" | sed -r "s|PPPPPPPP|$3|g" > "$P1/$DEBUGDIR/$LAUNCHNAME1.launch"
  cat ./arm_just-run.launch.template | sed -r "s|EEEEEEEE|$ELFRELPATH|g" | sed -r "s|PPPPPPPP|$3|g" > "$P1/$DEBUGDIR/$LAUNCHNAME2.launch"
  echo
  echo "Done"
fi
exit 0
